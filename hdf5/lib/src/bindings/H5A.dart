...
class H5ABindings {
  ...
  String getNameByIdx(loc_id, attr_idx){
    int attrSize = __getNameByIdx(loc_id, obj_name, H5_INDEX_NAME,
        H5_ITER_INC, attr_idx, nullptr, 0, H5P_DEFAULT);
    if (attrSize < 0) {
      throw Exception("Failed to get attribute name by index");
    }

    Pointer<Uint8> name = calloc<Uint8>(attrSize + 1);
    final status = __getNameByIdx(loc_id, obj_name, H5_INDEX_NAME,
        H5_ITER_INC, attr_idx, name, attrSize + 1, H5P_DEFAULT);
